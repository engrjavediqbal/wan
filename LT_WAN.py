...
from unet.model import unet_Model_LT_WAN, discriminator_LTA
import timeit
import util
...
SNAPSHOT_DIR = './snapshots/isprsPotsdam/rwanda2isprsPotsdam/LT_WAN/'
WEIGHT_DECAY = 1e-6
MODEL = 'Unet'
LOG_FILE = 'log'
...
def executeModelonTest(model, valDataset, data_dir):
    ...
    for j in range(len(linesValid)):
        ...
        output, _, _ = model.predict(test_image)
...
def main():
    ...
    generatorModel = unet_Model_LT_WAN(image_size, args.learning_rate, args.weight_decay)
    generatorModel.load_weights(args.src_trained_model)

    discrimnatorModel = discriminator_LTA(image_size)
...
#         for i in range(2):
                ...
                [_,_, xs] = generatorModel.predict(xs)
                xs = np.array(xs)
    ...
    #             print(histD)
                ...
                _, _, binCls = load_images(indicesTrainTar, args.batch_size, i, linesTar, image_size[0], args.data_dir)
                Xs_train, ys_train, binClsg = load_images(indicesTrain, args.batch_size, i, lines, image_size[0], args.data_dir)
                
    #             Gan.compile(loss=discrimnatorModel.loss, optimizer= discrimnatorModel.optimizer , metrics=discrimnatorModel.metrics )

                history = Gan.train_on_batch([Xs_train, Xt_train], [ys_train, yt_train, binCls])    
                t_loss.append(history[1])
                t_acc.append(history[3])
        ...
        if (epoch+1)%2 == 0:
            ...
            if mIoU > mIoUPrev:
                generatorModel.save(osp.join(args.snapshot_dir, 'seg_binCls_genModelbestWeights.h5'))
                discrimnatorModel.save(osp.join(args.snapshot_dir, 'seg_binCls_descModelbestWeights.h5'))
                mIoUPrev = mIoU
            ...
            if (epoch+1)%4 == 0:
                ...
                if mIoU >mIoUPrev2 and epoch>1:
                    generatorModel.save(osp.join(args.snapshot_dir, 'seg_binCls_genModelbestWeights_2nd.h5'))
                    discrimnatorModel.save(osp.join(args.snapshot_dir, 'seg_binCls_descModelbestWeights_2nd.h5'))
                    mIoUPrev2 = mIoU    
