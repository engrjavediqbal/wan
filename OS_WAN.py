...
from unet.model import unet_Model_CLS, discriminator_OSA
import timeit
import util
...
SNAPSHOT_DIR = './snapshots/isprsPotsdam/rwanda2isprsPotsdam/OS_WAN/'
WEIGHT_DECAY = 1e-6
MODEL = 'Unet'
LOG_FILE = 'log'
...
def  GANS(image_size, generatorModel, discrimnatorModel):
    ...
    [outPutSeg, binClsg] = generatorModel(inputSeg)
    
    [H, binCls] = generatorModel(inputs)
    ...
    GAN.compile(loss=['binary_crossentropy', 'binary_crossentropy', 'binary_crossentropy'], 
        optimizer=opt,
        metrics = ['accuracy'], loss_weights=[1.0, 0.10, 0.05])  
    return GAN
...
def main():
    ...
    generatorModel = unet_Model_CLS(image_size, args.learning_rate, args.weight_decay)
...
#         for i in range(2):
                ...
                [xs,_] = generatorModel.predict(xs)
                xs = np.array(xs)
