...
from unet.model import unet_Model_LT, discriminator_LTA
import timeit
import util
...
def  GANS(image_size, generatorModel, discrimnatorModel):
    ...
    outPutSeg,_ = generatorModel(inputSeg)
    
    _, H = generatorModel(inputs)
    ...
    GAN.compile(loss=['binary_crossentropy', 'binary_crossentropy'], 
        optimizer=opt,
        metrics = ['accuracy'], loss_weights=[0.8, 0.1]) 
    return GAN
...
def main():
    ...
    generatorModel = unet_Model_LT(image_size, args.learning_rate, args.weight_decay)
...
#         for i in range(2):
                ...
                _, xs = generatorModel.predict(xs)
                xs = np.array(xs)
