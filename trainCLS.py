...
from unet.model import unet_Model_CLS
import timeit
import util
...
def load_images(train_indices, batch_size, itr, lines, image_size, data_dir):
    ...
    for i in range(startInd, endInd):
        ...
        if binLabel.sum()>10:
            binClass_labels.append(1)
        else:
            binClass_labels.append(0)

    
    batch_images = np.array(batch_images)
    batch_labels = np.array(batch_labels, dtype=np.int)
    binClass_labels = np.array(binClass_labels, dtype=np.int)
    return batch_images, batch_labels, binClass_labels
...
def executeModelonTest(model, valDataset, data_dir):
    ...
    for j in range(len(linesValid)):
        ...
        test_image, y_train, _= load_images(indicesVal, 1, j, linesValid, image_size, data_dir)
        output, _ = model.predict(test_image)
...
def main():
    ...
    model = unet_Model_CLS(input_size, args.learning_rate, args.weight_decay)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i in range(iterations_t):
            X_train, y_train, binCls = load_images(indicesTrain, args.batch_size, i, lines, input_size[0], args.data_dir)
    #         print(X_train.shape)
            history = model.train_on_batch(X_train, [y_train, binCls])
            t_loss.append(history[0])
            seg_loss.append(history[1])
            cls_loss.append(history[2])
            seg_acc.append(history[3])
            cls_acc.append(history[4])
        t_loss = np.mean(np.array(t_loss, dtype=np.float32))
        seg_loss = np.mean(np.array(seg_loss, dtype=np.float32))  
        cls_loss = np.mean(np.array(cls_loss, dtype=np.float32))
        seg_acc = np.mean(np.array(seg_acc, dtype=np.float32)) 
        cls_acc = np.mean(np.array(cls_acc, dtype=np.float32))  
        print("Epoch: {}  Training Loss: {}  Seg Loss: {} binClass Loss: {}".format(epoch, t_loss, seg_loss, cls_loss))
        print("Seg Acc: {} binClass Acc: {}".format(seg_acc, cls_acc))
        ## validation
        for j in range(iterations_v):
            X_valid, y_valid, binCls = load_images(indicesVal, 1, j, lines1, input_size[0], args.data_dir)
            [a, b,c,d,e] = model.evaluate(X_valid, [y_valid, binCls], verbose=0)
        ...
        sv_loss = np.mean(np.array(sv_loss, dtype=np.float32))  
        cv_loss = np.mean(np.array(cv_loss, dtype=np.float32))
        sv_acc = np.mean(np.array(sv_acc, dtype=np.float32)) 
        cv_acc = np.mean(np.array(cv_acc, dtype=np.float32))  
        print("Epoch: {} Valid Loss: {}  SegValid Loss: {} ClsValid Loss: {}".format(epoch, v_loss, sv_loss, cv_loss))
        print("SegValid Acc: {} ClsValid Acc: {}".format(sv_acc, cv_acc))
        ...
        if (epoch+1)%2 == 0:
            ...
            if mIoU > mIoUPrev:
                model.save(osp.join(args.snapshot_dir, 'seg_binCls_bestWeights8_256_8MU.h5'))
                mIoUPrev = mIoU
            ...
            if (epoch+1)%4 == 0:
                ...
                if mIoU >mIoUPrev2 and epoch>1:
                    model.save(osp.join(args.snapshot_dir, 'seg_binCls_bestWeights8_256_8MU_2nd.h5'))
                    mIoUPrev2 = mIoU    
